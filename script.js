...
function updateStorageUsage() {
  if ('storage' in navigator && 'estimate' in navigator.storage) {
    navigator.storage.estimate().then(function (estimate) {
      ...
      const maximumAvailableElement = document.getElementById('maximum-available');

      currentUsageElement.textContent = formatStorage(estimate.usage);
      maximumAvailableElement.textContent = formatStorage(estimate.quota);
    ...
    // Optionally, update the display to indicate that the API is not supported
    const currentUsageElement = document.getElementById('current-usage');
    const maximumAvailableElement = document.getElementById('maximum-available');

    currentUsageElement.textContent = 'N/A';
    maximumAvailableElement.textContent = 'N/A';
  }
}
...
const audioSources = ['media/audio1.mp3'];
